...
def extract_info(string):
	'''
	receive some information from matalb app
	it contains "simulink_finish_flag / disturbance_instant_P1 / disturbance_finish_instant_P1"
	outputs: int(simulink_finish_flag), int(disturbance_instant_P1), int(disturbance_finish_P1)
	'''
	args = string.split()
...
def is_actuation_interval_changed(flow_info, disturbance_sec):
	...
	'''
	global prev_pkt_arriv_inst, broad_intv
	time = t.timeStr()
...
Task_periods_dist = [11, 11]
Task_maxTx_interf = [8, 8]
...
Interference_time = 3000	# 100=>1s  , 1=>10ms
Interference_finish = 5000
Disturbance_time = 50000000
Disturbance_finish = 15000000
interf_flag = 0
	...
	try:
		...
		while True:
			...
			if data:
				...
				simulink_finish_flag, Disturbance_time, Disturbance_finish = extract_info(data)
				...
				Disturbance_finish = Disturbance_finish * 100
				...
				if t.time() > 97656250*Interference_time and t.time() < 97656250*Interference_finish and interf_flag == 0:
					...
					interf_flag = 1
				# Interference handle
				if t.time() > 97656250*(Interference_time+1000) and t.time() < 97656250*Interference_finish and interf_handle_flag == 0:
					print("--------Interference handle-----------")
					interf_handle_flag = 1
					Task_maxTx = Task_maxTx_interf
					# Send a Interference signal to MAC Layer
					t.sendNumTx(Task_maxTx[0], Task_maxTx[1], 0, interf_flag)
				# Wireless Interference Finish
				if t.time() > 97656250*Interference_finish and interf_flag == 1:
					print("--------Interference Finish-----------")
					noise_offset = 12
					for node in Allnodes:
						m = t.getNode(node);
						for channel in [22, 23, 24, 25, 26]:
				...
				if t.time() > 97656250*(Interference_finish + 10) and interf_handle_flag == 1:
					print("--------Interference Finish handle-----------")
					interf_handle_flag = 0;
					Task_maxTx = Task_maxTx_orig
					# Send a Interference signal to MAC Layer
					t.sendNumTx(Task_maxTx[0], Task_maxTx[1], 0, interf_flag)
				...
				while (t.time() <= 97656250*run_count): # here the 97656250 represents 10ms
					...
					if rcved == 2:		# 0000 0010 - flowid:1
						...
						rcvedlist[7] = 1;
				rcvedlist[4] = broad_intv	# temparaly, broadcasting interval
				...
				if simulink_finish_flag == 1:	# Simulink finish
					connection.close()
					exit(0)
			else:
				break
