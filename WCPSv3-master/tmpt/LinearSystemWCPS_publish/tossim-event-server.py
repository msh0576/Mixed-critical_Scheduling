        ...
        while True:
            ...
            if data:
            	...
            	while (t.time() <= 97656250*run_count): # here the 97656250 represents 10ms
            		...
            		if rcvsendnodeid == 11:
            			rcvedSendNodeidList[0] = 11;

                print"rcvedlist[0]:" +str(rcvedlist[0]) +"   rcvsendnodeid:" +str(rcvedSendNodeidList[0])
            	run_count = run_count + 1;

            	reception = str(rcvedlist[0]) + ',' + str(rcvedlist[1]) + ',' + str(rcvedlist[2]) + ',' + str(rcvedlist[3]) + \
            	',' + str(rcvedlist[4]) + ',' + str(rcvedlist[5]) + ',' + str(rcvedlist[6]) + ',' + str(rcvedlist[7]) + str(rcvedSendNodeidList[0]);
				#t.setEvent(data)
            	#print reception
